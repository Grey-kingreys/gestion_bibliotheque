{{-- Chemin: resources/views/admin/livres/index.blade.php --}}
@extends('layouts.app')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8 flex justify-between items-center">
        ...
        <a href="{{ route('admin.livres.create') }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-lg font-medium transition inline-flex items-center">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-green-50 dark:bg-green-900/20 rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-green-700 dark:text-green-300 mb-1">Disponibles</p>
        ...
        <div class="bg-red-50 dark:bg-red-900/20 rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-red-700 dark:text-red-300 mb-1">Empruntés</p>
                    <p class="text-3xl font-bold text-red-600">{{ $stats['empruntes'] }}</p>
                </div>
        ...
        <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg shadow p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-indigo-700 dark:text-indigo-300 mb-1">Catégories</p>
                    <p class="text-3xl font-bold text-indigo-600">{{ $stats['categories'] }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-6">
        <form method="GET" action="{{ route('admin.livres.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input 
                type="text" 
                name="search" 
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('admin.livres.index') }}" class="bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg transition">
                    ✖
                </a>
            </div>
        </form>
                ...
                @forelse($livres as $livre)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.livres.show', $livre) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 mr-3">Voir</a>
                            <a href="{{ route('admin.livres.edit', $livre) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 mr-3">Modifier</a>
                            <form method="POST" action="{{ route('admin.livres.destroy', $livre) }}" class="inline" onsubmit="return confirm('⚠️ SUPPRESSION DÉFINITIVE !\n\nVoulez-vous vraiment supprimer ce livre de la base de données ?\n\nCette action est IRRÉVERSIBLE.');">
                                @csrf
                                @method('DELETE')
